// nvcc DotProductGPUSharedMemAtomicAddPading2.cu -o DotProductGPUSharedMemAtomicAddPading2
...
__global__ void DotProductGPU(float *a, float *b, float *DotGPU, int n)
{
	...
	{	
		if (thread<fold/2)
		{
			c_sh[thread]=c_sh[thread]+c_sh[thread+fold/2];
		}
		fold=fold/2;
		__syncthreads();
	}
