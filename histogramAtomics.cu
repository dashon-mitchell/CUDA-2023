...
__global__ void sharedfillHistogramGPU(float *randomNumbers, int *hist)
{
	float breakPoint;
	int i, k, done;
	float stepSize = MAX_RANDOM_NUMBER/(float)NUMBER_OF_BINS;
...
__global__ void GlobalfillHistogramGPU(float *randomNumbers, int *hist)
{
	float breakPoint;
	int i, k, done;
	float stepSize = MAX_RANDOM_NUMBER/(float)NUMBER_OF_BINS;
...
int main()
{
	...
	sharedfillHistogramGPU<<<GridSize,BlockSize>>>(RandomNumbersGPU, HistogramGPU);
