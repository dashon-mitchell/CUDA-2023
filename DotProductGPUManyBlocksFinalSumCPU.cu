// nvcc DotProductGPUManyBlocksFinalSumCPU.cu -o DotProductGPUManyBlocksFinalSumCPU
...
__global__ void DotProductGPU(float *a, float *b, float *c, int n)
{
	...
	while (fold>=2)
	{
		if(fold%2==1)
		{
			...
			{
				c[blockDim.x*blockIdx.x]=c[blockDim.x*blockIdx.x]+c[blockDim.x*blockIdx.x+fold-1];
			}
		fold--;
		}	

		if (thread<fold/2 && (id+fold/2)<n)
		{
