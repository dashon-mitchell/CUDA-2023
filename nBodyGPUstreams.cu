...
__global__ void getForces(float t, float4 *pos, float4 * force, int j)
{
	int ii;
...
void n_body()
{
	...
	{	
    		...
    		for(int j=0; j<Grid.x;j+=2)
    		{
			getForces<<<Grid, Block,0,Stream0>>>(time, PositionGPU, ForceGPU,j);
			getForces<<<Grid, Block,0,Stream1>>>(time, PositionGPU, ForceGPU,j+1);
		}
